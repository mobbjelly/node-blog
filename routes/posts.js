/**
 * Created by Joey on 2016/4/4 0004.
 */
...
router.post('/add', function (req, res) {
  ...
  if (errors) {
    ...
    posts.insert({
      ...
      if (err) {
        res.send("There was an issue when submitting the post");
      } else {
        req.flash("alert alert-success", "Post Submitted");
        res.location('/');
        res.redirect('/');
      }
    });
  ...
  if (errors) {
    ...
    posts.update({
      ...
      if (err) throw err;
        ...
        req.flash('alert alert-success', 'Comment Added');
